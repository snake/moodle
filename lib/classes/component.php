...
';
    ...
    protected static function fetch_subtypes($ownerdir) {
        ...
        if (file_exists("$ownerdir/db/subplugins.json")) {
            ...
            if (json_last_error() === JSON_ERROR_NONE) {
                ...
                $subtypesregister['plugintypes'] = !empty($subpluginsjson->plugintypes) ? (array) $subpluginsjson->plugintypes : [];
                $subtypesregister['deprecatedplugintypes'] = !empty($subpluginsjson->deprecatedplugintypes) ?
                    (array) $subpluginsjson->deprecatedplugintypes : [];
                $subtypesregister['deletedplugintypes'] = !empty($subpluginsjson->deletedplugintypes) ?
                    (array) $subpluginsjson->deletedplugintypes : [];
            } else {
     ...
     * @return array as (string)pluginname => (string)fulldir
     */
    public static function get_all_plugins_list(string $plugintype) {
        self::init();
        return array_merge(
