...
EOD;
    ...
    public function test_to_config_full(): void {
        ...
        $this->assertEquals(\core_ltix\constants::LTI_VERSION_1P3, $config->lti_ltiversion);
        ...
        $this->assertEquals(\core_ltix\constants::LTI_SETTING_DELEGATE, $config->lti_acceptgrades);
        ...
        $this->assertEquals(\core_ltix\constants::LTI_SETTING_ALWAYS, $config->lti_sendname);
        $this->assertEquals(\core_ltix\constants::LTI_SETTING_ALWAYS, $config->lti_sendemailaddr);
    ...
    public function test_to_config_minimal(): void {
        ...
        $this->assertEquals(\core_ltix\constants::LTI_SETTING_NEVER, $config->lti_acceptgrades);
        $this->assertEquals(0, $config->ltixservice_memberships);
        $this->assertEquals(\core_ltix\constants::LTI_SETTING_NEVER, $config->lti_sendname);
        $this->assertEquals(\core_ltix\constants::LTI_SETTING_NEVER, $config->lti_sendemailaddr);
    ...
    public function test_config_to_registration_lti11(): void {
        ...
        $lti = $reg['https://purl.imsglobal.org/spec/lti-tool-configuration'];
        $this->assertEquals(\core_ltix\constants::LTI_VERSION_1, $lti['version']);
    ...
    public function test_config_to_registration_lti20(): void {
        ...
        $this->assertEquals($type['name'], $reg['client_name']);
        $lti = $reg['https://purl.imsglobal.org/spec/lti-tool-configuration'];
        $this->assertEquals(\core_ltix\constants::LTI_VERSION_2, $lti['version']);
        $this->assertEquals('LtiDeepLinkingRequest', $lti['messages'][0]['type']);
        $this->assertEquals('base.test.url', $lti['domain']);
        $this->assertEquals($type['baseurl'], $lti['target_link_uri']);
        $oauth = $lti['oauth_consumer'];
