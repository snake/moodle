...
/**
 * Integration tests for v1p1_resource_link_launch_service.
 *
 * @package    core_ltix
...
class v1p1_resource_link_launch_service_test extends \basic_testcase {
    ...
    public function test_launch_invokes_pipeline_and_request_builder(): void {
        ...
        $pipelinefactory->expects($this->once())
            ->method('create_for')
            ->with(
                $this->equalTo(lti_version::LTI_VERSION_1),
                $this->equalTo(message_type::create('basic-lti-launch-request', 'basic-lti-launch-request')),
            )
            ->willReturn($pipeline);

        $requestbuilder = $this->createMock(v1p1_resource_link_launch_request_builder::class);
        $requestbuilder->expects($this->once())
            ->method('build_message')
            ->with($toolconfig, $link, [])
