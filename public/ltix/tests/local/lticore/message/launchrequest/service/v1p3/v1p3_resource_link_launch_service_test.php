...
/**
 * Unit tests for v1p3_resource_link_launch_service.
 *
 * @package    core_ltix
...
class v1p3_resource_link_launch_service_test extends \basic_testcase {
    ...
    public function test_launch_invokes_pipeline_and_request_builder(): void {
        ...
        $pipelinefactory->expects($this->once())
            ->method('create_for')
            ->with(
                $this->equalTo(lti_version::LTI_VERSION_1P3),
                $this->equalTo(message_type::create('LtiResourceLinkRequest', 'LtiResourceLinkRequest')),
            )
            ->willReturn($pipeline);

        $requestbuilder = $this->createMock(v1p3_resource_link_launch_request_builder::class);
